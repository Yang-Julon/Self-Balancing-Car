...
int mpu_init(void)
{
    ...
    if (rev) {
        /* Congrats, these parts are better. 恭喜你这些零件更好了*/
        if (rev == 1)
        ...
        rev = data[0] & 0x0F;
        if (!rev) {
            log_e(" Product ID read as 0 indicates device is either "  //产品ID读取为0表示设备为““不兼容或MPU3050
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    /* Set to invalid values to ensure no I2C writes are skipped.设置为无效值以确保不跳过I2C写入 */
